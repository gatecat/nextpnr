        ...
        for (auto &n : nets) {
            ...
            if (count > 0) {
                ...
                for (auto axis : {Axis::X, Axis::Y}) {
                    n.pcost.at(axis) =
                            std::sqrt(1.0 + n.delta_sum.at(axis));
                }
            }
            // timing cost
            n.tcost = RealLoc(0, 0);
            ...
            if (n.ni->driver.cell) {
                ...
                for (auto axis : {Axis::X, Axis::Y}) {
                    ...
                    n.tcost.at(axis) = std::sqrt(n.tmg_deltasum.at(axis));
                }
            }
        }
    }
    ...
    void do_solve(Axis axis)
    {
        ...
        for (auto &m : macros) {
            ...
            for (auto cell_idx : m.cells) {
                ...
                c.r.at(axis) = omega * c.r.at(axis) + (1 - omega) * (pos + (axis == Axis::Y ? c.macro_offset.y : c.macro_offset.x));
